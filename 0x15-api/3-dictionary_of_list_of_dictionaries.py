...
def helper_todoall(username, tasks):
    '''a helper function for the todo_all_employees() function'''
    list_ = []
    for task in tasks:
        title = task["title"]
        status = task["completed"]
...
def todo_all_employees():
    '''éxport the todo tasks of all employees to a json file'''
    base_url = 'https://jsonplaceholder.typicode.com'
    ...
    for user in users:
        ...
        todo_url = f'{base_url}/todos?userId={id_}'
        tasks = requests.get(todo_url).json()
        all_dict[f"{id_}"] = helper_todoall(username, tasks)
